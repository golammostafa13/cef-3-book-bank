...
/**
 * Admin chrome: a slim icon rail plus a topbar, following the reference
 * dashboard. Everything under /admin is gated twice — once by the route guard
 * in `proxy.ts` and again inside every Server Action — so the identity shown
 * here is the real signed-in librarian.
 */
        ...
        <header className="sticky top-0 z-30 flex h-[74px] items-center gap-4 border-b border-line/60 bg-surface px-6">
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <div className="ml-2 flex items-center gap-2.5 border-l border-line pl-4">
              {librarian?.picture ? (
                /* Google's avatar host is not in `images.remotePatterns`, and a
                   single 36px avatar is not worth an optimiser round trip. */
              ...
              <div className="hidden leading-tight sm:block">
                ...
                <p className="max-w-40 truncate text-xs text-ink-faint">
                  {librarian?.email ?? dict.admin.signedIn}
                </p>
              </div>
            </div>
          </div>
